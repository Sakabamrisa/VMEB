...
from transformers import Qwen2_5_VLForConditionalGeneration,LlavaNextVideoForConditionalGeneration,AutoProcessor
...
class MultimodalEditor:
    ...
    def __init__(self,
        ...
        self.sentence_transformer = SentenceTransformer('hugging_cache/all-MiniLM-L6-v2')
        ...
        if type(self.model_name) is str:
            if hparams.model_name == "blip2":
            ...
            elif "qwen-vl" == hparams.model_name.lower():
            ...
            elif hparams.model_name == "Qwen2.5-VL":
                ...
                # print(model.config._attn_implementation)  # 确认输出 "flash_attention_2"
            ...
            if hparams.model_name in  ["blip2", "minigpt4"]:
                ...
                self.vis_tok = vis_processor
            elif hparams.model_name.lower() == "qwen2.5-vl" or hparams.model_name.lower() == "llava-next-video":
                vis_processor = AutoProcessor.from_pretrained(hparams.name)
                self.vis_tok = vis_processor
            else:
                raise NotImplementedError(f"Model {hparams.model_name} not supported")

            if hparams.model_name.lower() == "qwen2.5-vl" or hparams.model_name.lower() == "llava-next-video":
                self.tok = self.vis_tok
            elif (hparams is not None and hasattr(hparams, 'tokenizer_name')):
                tok_name = (
        ...
        self.vis_root = hparams.coco_video
        self.rephrase_root = hparams.rephrase_video
    ...
    def edit(self,
            ...
            rephrase_video: Optional[Union[str, List[str]]] = None,
        ...
        if isinstance(prompts, List):
            assert len(prompts) == len(targets) == len(video)
        else:
            prompts, targets, video = [prompts,], [targets,], [video,]

        if hasattr(self.hparams, 'batch_size'):  # For Singleton Editing, bs=1
            self.hparams.batch_size = 1

        requests = self._prepare_requests(prompts, targets, video, rephrase_prompts, rephrase_video, locality_inputs,
                                          **kwargs)
        ...
        for i, request in enumerate(requests):
            ...
            if self.alg_name == 'IKE':
                ...
                edited_model, weights_copy, icl_examples = self.model, {}, self.apply_algo(
                    ...
                    sentence_model=self.sentence_transformer,
        ...
        for i, request in tqdm(enumerate(ds), desc='Editing dataset', total=len(ds), ncols=120):
            ...
            if self.alg_name == 'IKE':
                ...
                port = {
                    ...
                    'port_input': ' '.join([request["prompt"], request["target"]]),#因为不需要portability，所以直接随便给参数
                    'port_acc': metrics['post']['portability_acc'],
    ...
    def _prepare_requests(self,
        ...
        for prompt, target, video_ in zip(prompts, targets, video)
        ]
        ...
        if rephrase_video is not None:
            if isinstance(rephrase_video, str):
                rephrase_image = [rephrase_video, ]
