...
def build_prompt(hparams, question, answer, no_video=False):
    if  hparams.model_class == "Qwen2_5_VLForConditionalGeneration":
        ...
        return prompt
    elif hparams.model_class == "LlavaNextVideoForConditionalGeneration":
        VIDEO_TOKEN = "<video>"
        if no_video == True:
...
def compute_icl_multimodal_edit_quality(
    ...
    :return: Dictionary containing rewriting metrics
    """
    vis_root = hparams.coco_video
    rephrase_root = hparams.rephrase_video
    # First, unpack rewrite evaluation record.
    target = record["target"]
    prompt = record["prompt"]
    image = record['image'].to(hparams.device) if torch.is_tensor(record['image']) and not record['image'].is_cuda else record['image']
    #when you use llava,this isn't needed
    video = record['video']
    rephrase = record["rephrase_prompt"] if 'rephrase_prompt' in record.keys() else None
    rephrase_image = record["image_rephrase"] if 'image_rephrase' in record.keys() else None
    if hparams.model_name.lower() == 'qwen2.5-vl':
        rephrase_video = record["rephrase_video"] if 'video_rephrase' in record.keys() else None
    elif hparams.model_name.lower() == 'llava-next-video':
        rephrase_video = record["video_rephrase"] if 'video_rephrase' in record.keys() else None
    ...
    if "multimodal_locality_image" in record.keys():
        ...
        m_loc_video = record['multimodal_locality_video']
    ...
    if pre_edit:
        edit_acc, _ = icl_multimodal_lm_eval(model, model_name, hparams, tok, icl_examples,
                                       target, prompt, image, video)
    else:
        edit_acc, _ = icl_multimodal_lm_eval(model, model_name, hparams, tok, icl_examples,
                                              target, new_fact, image, video)
    ret = {
        f"rewrite_acc": edit_acc
    }
    if rephrase is not None:
        rephrase_acc, _ = icl_multimodal_lm_eval(model, model_name, hparams, tok, icl_examples,
                               target, f'New Fact: {prompt} {target}\nPrompt: {rephrase}', image, video)
        ret['rephrase_acc'] = rephrase_acc
        
    if "image_rephrase" in record.keys():
        rephrase_image_acc, _ = icl_multimodal_lm_eval(model, model_name, hparams, tok, icl_examples,
                               target, new_fact, rephrase_image, rephrase_video)
        ret['rephrase_image_acc'] = rephrase_image_acc

    if "video_rephrase" in record.keys():
        rephrase_video_acc, _ = icl_multimodal_lm_eval(model, model_name, hparams, tok, icl_examples,
                               target, new_fact, rephrase_image, rephrase_video)
        ret['rephrase_video_acc'] = rephrase_video_acc
    ...
    if not pre_edit:
        ...
        if "multimodal_locality_image" in record.keys():
            pre_image_loc_logits = icl_multimodal_loc_logits(model, model_name, hparams, tok, [''], m_loc_a, m_loc_q, m_loc_image, m_loc_video)
            post_image_loc_logits = icl_multimodal_loc_logits(model, model_name, hparams, tok, icl_examples, m_loc_a, f'New Fact: {prompt} {target}\nPrompt: {m_loc_q}', m_loc_image, m_loc_video)
        ...
        if "multimodal_locality_video" in record.keys():
            pre_video_loc_logits = icl_multimodal_loc_logits(model, model_name, hparams, tok, [''], m_loc_a, m_loc_q, m_loc_image, m_loc_video)
            post_video_loc_logits = icl_multimodal_loc_logits(model, model_name, hparams, tok, icl_examples, m_loc_a, f'New Fact: {prompt} {target}\nPrompt: {m_loc_q}', m_loc_image, m_loc_video)

            pre_video_loc_logits_softmax_top_k = torch.topk(torch.nn.functional.softmax(pre_video_loc_logits.float(), dim=-1), k=10, dim=-1).indices
            post_video_loc_logits_softmax_top_k = torch.topk(torch.nn.functional.softmax(post_video_loc_logits.float(), dim=-1), k=10, dim=-1).indices

            locality_video_acc = sum(post_video_loc_logits_softmax_top_k.view(-1) == pre_video_loc_logits_softmax_top_k.view(-1))/post_video_loc_logits_softmax_top_k.view(-1).shape[0]

            ret['locality_video_acc'] = locality_video_acc
    ###################################################################
...
def icl_multimodal_lm_eval(
    ...
    samples = prepare_multimodal_edit(hparams, tokenizer, target, [''.join(icl_examples) + f'{x}'], image, video)
...
def icl_multimodal_loc_logits(
    ...
    batch = prepare_multimodal_edit(hparams, tokenizer, target, [''.join(icl_examples) + f'{x}'], image, video)
...
def prepare_multimodal_edit(hparams,
    ...
    if isinstance(prompts, str):
        prompts = [prompts,]
    if "qwen-vl" == hparams.model_name.lower():
        ret = {
    ...
    elif "qwen2.5-vl" == hparams.model_name.lower():

        # 确保文本列表格式正确
        text_inputs = []
        if isinstance(prompts[0], str) and isinstance(target[0], str):
            # 如果是单个字符串，创建一个包含一个元素的列表
            text_inputs = [build_prompt(hparams, prompts[0], target[0])]
        else:
            # 如果已经是可迭代对象，正常处理
            text_inputs = [build_prompt(hparams, r, t) for r, t in zip(prompts[0], target[0])]

        ret = {
            'inputs': tok(
                text=text_inputs,
                images=None,
                videos=video,  # 使用处理后的视频
                fps=1.0,
                padding=True,
                return_tensors='pt',
                max_frames=16
            ).to(hparams.device),
            'labels': tok.tokenizer(target[0], padding=True, truncation=True, max_length=32, add_special_tokens=False,
                                    return_tensors="pt")["input_ids"]
        }

    elif "llava-next-video" == hparams.model_name.lower():
        ...
        ret = {
            'inputs': tok(
                text = build_prompt(hparams, SYSTEM_PROMPT + prompts[0], target[0]),
                padding = True,
                videos = video,
                return_tensors='pt',
            ).to(hparams.device),
            'labels': tok.tokenizer(target[0], add_special_tokens=False, return_tensors="pt",)["input_ids"]
        }
