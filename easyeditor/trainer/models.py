...
import transformers
from transformers import GPT2Tokenizer, GPT2TokenizerFast, AutoModel, AutoModelForCausalLM, AutoTokenizer,LlavaNextVideoForConditionalGeneration,LlavaNextVideoProcessor,Qwen2_5_VLForConditionalGeneration, LlavaOnevisionForConditionalGeneration
...
def get_model(config):
    if config.model_class == "BertClassifier":
        ...
        model = LlavaNextVideoForConditionalGeneration.from_pretrained(
            ...
            #load_in_4bit=True,
            #attn_implementation="flash_attention_2" FT时关掉
        ).to(0)
    elif config.model_name.lower() == "video-utr":
        LOG.info(
            f"Loading model with name {config.model_name}"
        )
    ...
    elif config.model_name == "Qwen2.5-VL":
        LOG.info(
            f"Loading model with name {config.model_name}"
        )
