...
class SERAC_MULTI(EditableModel):
    def __init__(self, model, config, model_constructor, classifier=None, classifier_tok=None,
        ...
        if replacement is None:
            if config.model_name.lower() == "qwen2.5-vl":
                self.processor = transformers.AutoProcessor.from_pretrained(config.small_name)
                self.replacement_tok = self.processor.tokenizer  # 使用处理器中的tokenizer
            elif config.model_name.lower() == "llava-next-video":
                # 加载处理器和分词器
                self.processor = transformers.AutoProcessor.from_pretrained(config.small_name)
                self.replacement_tok = self.processor.tokenizer  # 使用处理器中的tokenizer
            ...
            if self.config.freeze_cntr:
                ...
                if config.model_class == "BertClassifier":
                ...
                elif config.model_name.lower() == "qwen2.5-vl":
                    # 加载Qwen2.5-VL模型作为反事实模型
                    self.replacement = transformers.Qwen2_5_VLForConditionalGeneration.from_pretrained(
                        config.small_name,
                    ...
                    for k, v in self.replacement.named_parameters():
                        if 'model.layers.27' in k:  # 只训练最后一层，Qwen2.5-VL有28层decoder
                            v.requires_grad = True
                        else:
                            v.requires_grad = False
                elif config.model_name.lower() == "llava-next-video":
                    # 加载LLaVA-NeXT-Video模型作为反事实模型
                    self.replacement = transformers.LlavaNextVideoForConditionalGeneration.from_pretrained(
                        config.small_name,
                    ...
                    # 只训练模型的最后几层，提高效率
                    for k, v in self.replacement.named_parameters():
                        if 'model.layers.31' in k:  # 假设LLaVA-NeXT-Video有32层，只训练最后一层
                            v.requires_grad = True
                        else:
                            v.requires_grad = False
                ...
                if self.replacement_tok.sep_token is None and "gpt" not in config.name.lower() and "qwen" not in config.model_name.lower():
                    add_sep(self.replacement_tok, self.replacement)
                if self.replacement_tok.pad_token is None and "qwen" not in config.model_name.lower():
        ...
        elif config.model_name == "llava" or config.model_name == "qwen-vl" or config.model_name == "owl-2" or config.model_name.lower() == "qwen2.5-vl" or config.model_name.lower() == "llava-next-video":
    ...
    def build_prompt(self, question, answer, no_video = False):
        ...
        # Qwen2.5-VL的prompt构建函数
            ...
            return prompt
        elif self.config.model_name.lower() == "llava-next-video":
            # LLaVA-NeXT-Video的prompt构建格式
            VIDEO_TOKEN = "<video>"
            if no_video:
                prompt = f"USER: {question}\nASSISTANT: {answer}"
            else:
                prompt = f"USER: {VIDEO_TOKEN} {question}\nASSISTANT: {answer}"
            return prompt
    ...
    def state_dict(self, destination=None, prefix="", keep_vars=False):
        ...
        elif self.config.model_name == "minigpt4" or self.config.model_name == "llava" or self.config.model_name == "qwen-vl" or self.config.model_name == "owl-2" or self.config.model_name.lower() == "qwen2.5-vl":
            for k in self.replacement.state_dict().keys():
                if 'model.layers.27' not in k and self.config.model_name.lower() == "qwen2.5-vl":
                    del state_dict[f"replacement.{k}"]
                elif '31' not in k and self.config.model_name.lower() != "qwen2.5-vl":
                    del state_dict[f"replacement.{k}"]
        elif self.config.model_name.lower() == "llava-next-video":
            for k in self.replacement.state_dict().keys():
                if 'model.layers.31' not in k:  # 只保存我们训练的参数
                    del state_dict[f"replacement.{k}"]
    ...
    def load_state_dict(self, state_dict, strict: bool = True):
        ...
        def ok_to_miss(k):
            if self.config.model_name.lower() == "qwen2.5-vl":
                return k.startswith("model.") or (self.config.freeze_cntr and k.startswith("replacement.")) or (k.startswith("replacement.") and ("model.layers.27" not in k))
            elif self.config.model_name.lower() == "llava-next-video":
                return k.startswith("model.") or (self.config.freeze_cntr and k.startswith("replacement.")) or (k.startswith("replacement.") and ("model.layers.31" not in k))
    ...
    def outer_parameters(self, grouped=False):
        ...
        if not self.config.freeze_cntr:
            ...
            if self.config.model_name.lower() == "qwen2.5-vl":
                params_extend = []
                # 为Qwen2.5-VL模型，只训练最后一层
                for k, v in self.replacement.named_parameters():
                    if 'model.layers.27' in k:  # 只训练最后一层
                        params_extend.append(v)
                model_params.extend(params_extend)
            elif self.config.model_name.lower() == "llava-next-video":
                params_extend = []
                # 为LLaVA-NeXT-Video模型，只训练最后一层
                for k, v in self.replacement.named_parameters():
                    if 'model.layers.31' in k:  # 只训练最后一层
                        params_extend.append(v)
                model_params.extend(params_extend)
            elif self.config.model_name == "minigpt4" or self.config.model_name == "llava" or self.config.model_name == "qwen-vl" or self.config.model_name == "owl-2":
                params_extend = []
    ...
    def edit(self, batch, condition=None, detach_history=False):
        ...
        if isinstance(labels, torch.Tensor):
            ...
            elif self.config.model_name.lower() == "qwen2.5-vl" or self.config.model_name.lower() == "llava-next-video":
                labels = detokenize(labels, self.processor.tokenizer)
                labels = [s.strip() for s in labels]
            else:
    ...
    def build_rep_input_tokens(self, kwargs, idxs, generation=False):
        ...
        elif "inputs" in kwargs and kwargs["inputs"] is not None:
            assert len(idxs) == len(kwargs["inputs"]["input_ids"]), "Need one cache idx for each test input"
        ...
        if "qwen-vl" == self.config.model_name.lower() or "owl-2" == self.config.model_name.lower():
            rep_input_tokens = self.replacement_tok(rep_texts, return_tensors="pt", add_special_tokens=True).to(self.config.device)
        elif "qwen2.5-vl" == self.config.model_name.lower() or "llava-next-video" == self.config.model_name.lower():
            rep_input_tokens = self.replacement_tok(rep_texts, return_tensors="pt", add_special_tokens=True).to(self.config.device)
        else:
        ...
        if self.config.model_name == "minigpt4" or self.config.model_name == "blip2" or self.config.model_name == "llava" or self.config.model_name.lower() == "qwen2.5-vl" or "llava-next-video" == self.config.model_name.lower():
        ...
        with torch.no_grad():
            if len(self.cache_inputs) == 0:
                ...
                if self.config.model_name == "blip2":
                    ...
                    base_logits = super().forward(**inputs[0]['inputs'], **kwargs)
                    if not isinstance(base_logits, torch.Tensor):
                        base_logits = base_logits.logits
                    base_logits = base_logits.float()
        ...
        if self.config.freeze_cntr:
            ...
            if self.config.model_name == "blip2":
                ...
                if image is not None:
                    ...
                    image_token_ids = torch.ones((input_ids.shape[0]), dtype=input_ids.dtype,
                                                 device=input_ids.device).fill_(IMAGE_TOKEN_INDEX)
                    ...
                    attention_mask = torch.cat(
                        (attention_mask[:, :1], image_att_mask.unsqueeze(1), attention_mask[:, 1:]), dim=1)

                    targets = torch.cat((rep_cls_labels[:, :1], image_token_ids.unsqueeze(1), rep_cls_labels[:, 1:]),
                                        dim=1)
                ...
                rep_cls_labels = rep_cls_inputs.pop("labels")
                rep_cls_logits = self.replacement(**inputs[0]['inputs'])['logits'] #本身就包含视觉信息，没必要专门处理了
                rep_cls_logits = rep_cls_logits[:, -base_probs.shape[1]:, :]
