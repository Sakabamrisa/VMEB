    ...
    def forward(self, *inputs, **kwargs):
        if 'minigpt4' in self.config.model_name.lower() or 'blip' in self.config.model_name.lower() or 'llava' == self.config.model_name.lower():
            ...
            outputs = _logits(self.model.train()(input_ids, 
                                        images=image))
        elif 'llava-next-video' == self.config.model_name.lower() or 'qwen2.5-vl' == self.config.model_name.lower():
    ...
    def edit(self, batch, condition=None, detach_history=False, return_factors=False):
        ...
        if 'minigpt4' in self.config.model_name.lower() or 'blip' in self.config.model_name.lower() or 'llava' == self.config.model_name.lower():
            ...
            for it in pbar:
                ...
                opt.step()
        elif "llava-next-video" in self.config.model_name.lower() or "qwen2.5-vl" in self.config.model_name.lower():
            pbar = trange(self.config.num_steps, ncols=120)
            for it in pbar:
                opt.zero_grad()


                outputs = self.model(**batch['inputs'].to(self.config.device))
                if not isinstance(outputs, torch.Tensor):
                    outputs = outputs.logits
                loss = self.edit_loss_fn(self.config, outputs, batch["labels"])["nll"]
                pbar.set_postfix({"loss": loss.item()})
