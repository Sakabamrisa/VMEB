...
class MEND(EditableModel):
    ...
    def forward(self, *inputs, **kwargs):
        if 'minigpt4' in self.config.model_name.lower() or 'blip' in self.config.model_name.lower() or 'llava' == self.config.model_name.lower():
            outputs = self.model(*inputs, **kwargs)
        elif 'qwen-vl' == self.config.model_name.lower():
        ...
        elif 'llama' == self.config.model_name.lower():
            outputs = _logits(self.model(input_ids=kwargs['input_ids'], attention_mask=kwargs['attention_mask']))
            # outputs = outputs[:, -kwargs['labels'].shape[-1]:, :]
            ...
            outputs = _logits(self.model(input_ids=kwargs['input_ids'], attention_mask=kwargs['attention_mask']))
            # outputs = outputs[:, -kwargs['labels'].shape[-1]:, :]
        elif 'qwen' == self.config.model_name.lower():
            outputs = _logits(self.model(input_ids=kwargs['input_ids'], attention_mask=kwargs['attention_mask']))
            # outputs = outputs[:, -kwargs['labels'].shape[-1]:, :]
        elif 'llava-next-video' == self.config.model_name.lower():
            #outputs = _logits(self.model(inputs[0]['inputs']))
            outputs = _logits(self.model(**inputs[0]['inputs']))
            #outputs = _logits(self.model(input_ids=kwargs['inputs']['input_ids'], attention_mask=kwargs['inputs']['attention_mask'], pixel_values_videos=kwargs['video']))
        elif 'qwen2.5-vl' in self.config.model_name.lower():
            outputs = _logits(self.model(**inputs[0]['inputs']))
        elif 'video-utr' in self.config.model_name.lower():
            outputs = _logits(self.model(**inputs[0]['inputs']))
        else:
    ...
    def edit(self, batch, vis_processor, condition=None, detach_history=False, return_factors=False):
        if 'minigpt4' in self.config.model_name.lower() or 'blip' in self.config.model_name.lower() or 'llava' == self.config.model_name.lower():
            ...
            outputs = _logits(self.model(**batch['inputs'].to(self.config.device)))
            loss_dict = self.edit_loss_fn(self.config, outputs, batch["labels"].to(self.config.device))
            #print("预测的ID:", vis_processor.tokenizer.decode(loss_dict['pred_ids'][0]))  # 打印pred_ids
            #print("目标ID:", vis_processor.tokenizer.decode(loss_dict['targ_ids'][0]))  # 如果需要，也可以打印目标ID
            #print("完整ID：",vis_processor.tokenizer.decode(loss_dict['pred_all'][0]))
            loss = loss_dict["nll"]  # 继续使用nll值
        elif 'qwen2.5-vl' in self.config.model_name.lower():
            outputs = _logits(self.model(**batch['inputs'].to(self.config.device)))
            loss = self.edit_loss_fn(self.config, outputs, batch["labels"].to(self.config.device))["nll"]
        elif 'video-utr' in self.config.model_name.lower():
            outputs = _logits(self.model(**batch['inputs'].to(self.config.device)))
            loss = self.edit_loss_fn(self.config, outputs, batch["labels"].to(self.config.device))["nll"]
        else:
            outputs = _logits(self.model(**batch))
            loss_dict = self.edit_loss_fn(self.config, outputs, batch["labels"])
            print("预测的ID:", vis_processor.tokenizer.decode(loss_dict['pred_ids'][0]))  # 打印pred_ids
            print("目标ID:", vis_processor.tokenizer.decode(loss_dict['targ_ids'][0]))  # 如果需要，也可以打印目标ID
            loss = loss_dict["nll"]  # 继续使用nll值
